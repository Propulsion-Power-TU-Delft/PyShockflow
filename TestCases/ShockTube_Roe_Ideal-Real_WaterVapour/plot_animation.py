...
TIME_MAX = 0.45*LENGTH*(Rhocr/Pcr)**(0.5)
...
for iTime in range(sol.timeVec.shape[0]):
    ...
    ax[0, 0].plot(sol.xNodes, sol.solution['Density'][1:-1, iTime]/Rhocr, '-C0o', ms=2, label='ideal gas') 
    ax[0, 0].set_ylabel(r'$\rho / \rho_{cr}$')

    ax[0, 1].plot(sol.xNodes, sol.solution['Velocity'][1:-1, iTime], '-C0o', ms=2, label='ideal gas')
    ax[0, 1].set_ylabel(r'Velocity [m/s]')

    ax[1, 0].plot(sol.xNodes, sol.solution['Pressure'][1:-1, iTime]/Pcr, '-C0o', ms=2, label='ideal gas')
    ax[1, 0].set_ylabel(r'$P/P_{cr}$')
